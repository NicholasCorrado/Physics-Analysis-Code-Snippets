  ...
  for ( std::vector<Mdst_charged>::iterator it = ChargedMgr.begin(); it != ChargedMgr.end(); it++ ) {
		...
		if ( mdst_chrg.charge() > 0 ) {
			...
			if ( hep_chrg.get_ID() == m_mc_pim_id ) {
				m_count_pim_rec++;
				if ( fabs(dr_pi) <= dr_cut )               m_count_pim_good_dr++;
				if ( fabs(dz_pi) <= dz_cut )               m_count_pim_good_dz++;
				if ( pt > pt_cut )                         m_count_pim_good_pt++;
				if ( goodTrackQuality( mdst_chrg, "pi" ) ) m_count_pim_good_trk++;
			}

			else if ( hep_chrg.get_ID() == m_mc_mum_id ) {
				m_count_mum_rec++;
				if ( fabs(dr_mu) <= dr_cut )               m_count_mum_good_dr++;
				if ( fabs(dz_mu) <= dz_cut )               m_count_mum_good_dz++;
				if ( pt > pt_cut )                         m_count_mum_good_pt++;
				if ( goodTrackQuality( mdst_chrg, "mu" ) ) m_count_mum_good_trk++;
			}
		}
	}
}
